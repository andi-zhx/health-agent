...
一个基于 **Flask + PostgreSQL + 原生 HTML/JS** 的健康管理档案管理系统，适用于诊所/康复中心等场景。  
...
- 双击 `启动健康管理系统.bat`（显示命令行窗口）
- 双击 `启动健康管理系统.pyw`（无命令行窗口）
...
├─ README.md              # 项目说明
├─ 启动健康管理系统.bat       # Windows 启动脚本（推荐）
├─ 启动健康管理系统.pyw       # Windows 启动脚本（无控制台）
